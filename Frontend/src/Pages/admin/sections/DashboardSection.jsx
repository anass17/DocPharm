                                        ...
                                        recentActivities.map((item, index) => {
                                            return (
                                                item.type === 'order' ? (
                                                        ...
                                                        <div>
                                                            {
                                                                item.status === 'delivered' ? (
                                                                    <Link to={''} style={{ marginBottom: "0", color: PRIMARY_BLUE, display: 'block', fontWeight: 'bold' }}>An order has been delivered</Link>
                                                                ) : (
                                                                    item.status === 'ready' ? (
                                                                        <Link to={''} style={{ marginBottom: "0", color: PRIMARY_BLUE, display: 'block', fontWeight: 'bold' }}>An order is ready</Link>
                                                                    ) : (
                                                                        item.status === 'accepted' ? (
                                                                            <Link to={''} style={{ marginBottom: "0", color: PRIMARY_BLUE, display: 'block', fontWeight: 'bold' }}>A pharmacy has accepted your order</Link>
                                                                        ) : (
                                                                            <Link to={''} style={{ marginBottom: "0", color: PRIMARY_BLUE, display: 'block', fontWeight: 'bold' }}>You have placed an order</Link>
                                                                        )
                                                                    )
                                                                )
                                                            }
                                                    ...
                                                    <Flex key={'med-' + index} justify="space-between" style={{ marginBottom: 25 }}>
                                                        <div>
                                                            <Link to={''} style={{ marginBottom: "0", color: PRIMARY_BLUE, display: 'block', fontWeight: 'bold' }}>Dr. {item.first_name} {item.last_name}</Link>
